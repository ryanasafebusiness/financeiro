...
def with_plan(db):
    db.seed("plans", {
        "id": PLAN_ID, "name": "Mensal", "price": 19.9, "duration_days": 30,
        "message_limit": 0, "stripe_price_id": PRICE_ID, "active": True,
    })
    return db
...
def test_corpo_adulterado_401(with_plan):
    ...
    adulterado = payload.replace(b'"amount_total": 1990', b'"amount_total": 1')
...
def test_checkout_completed_registra_pagamento(with_plan):
    ...
    assert pays[0]["amount"] == 19.9        # centavos convertidos
